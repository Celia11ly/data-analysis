...
# 2. 积极配合调查 - 明确指定编码顺序，确保语义与编码一致
# '否'表示不配合(编码0)，'是'表示配合(编码1)，'未知'保持不变(编码2)
cooperation_mapping = {'否': 0, '是': 1, '未知': 2}
df_encoded['积极配合调查_编码'] = df_encoded['积极配合调查'].map(cooperation_mapping)

# 3. 其他分类变量 - 先进行OneHot编码处理
# 定义无序分类变量
nominal_vars = [
    '是否是组织者', '提供证据', '主动停止违法行为',
    '是否没收违法所得', '主动整改', '垄断行为性质',
...
key_vars_correlation = [
    ...
    ('是否是组织者_是', '是否是组织者'),
    ('提供证据_未知', '提供证据'),
    ('主动停止违法行为_未知', '主动停止违法行为'),
    ('是否没收违法所得_是', '是否没收违法所得'),
    ('主动整改_是', '主动整改'),
    ('垄断行为性质_滥用支配地位', '垄断行为性质'),
    ('垄断行为性质_纵向垄断协议', '垄断行为性质'),
    ('有无行业协会参与_有', '有无行业协会参与')
]
...
【编码方式说明】
- 本分析使用了正确的编码方式：
  - 有序分类变量（如社会危害程度）明确指定了编码顺序（未知=0，轻=1，重=2）
  - 积极配合调查变量明确指定了语义一致的编码（否=0，是=1，未知=2）
  - 无序分类变量使用了OneHotEncoder编码，避免了错误的顺序假设
- 这种编码方式确保了相关性分析结果准确反映变量间的实际关系

【统计学意义】
- 相关性强度标准：r≈0表示无相关，0<|r|<0.3表示弱相关，0.3≤|r|<0.7表示中等相关，|r|≥0.7表示强相关
- 本研究结果解释：
  - 社会危害程度与罚款比例呈中等强度正相关（r={target_correlations.get('社会危害程度_编码', 0):.4f}），说明社会危害程度越高，罚款比例可能越高，这与法律规定和执法实践一致
  - 违法行为持续时间与罚款比例呈弱至中等强度正相关（r={target_correlations.get('违法行为持续时间（月）', 0):.4f}），表明违法行为持续时间越长，罚款比例可能越高
  - 积极配合调查与罚款比例的相关方向和强度（r={target_correlations.get('积极配合调查_编码', 0):.4f}）需要结合编码方式理解，这里我们使用了语义一致的编码，确保了结果的准确性
"""

# ===== 更新报告 =====
# 读取现有的报告
with open('/Users/Celia/Downloads/dataA/improved_final_analysis_report_with_reliability_correlation.txt', 'r', encoding='utf-8') as f:
...
# 保存更新后的报告
with open('/Users/Celia/Downloads/dataA/improved_final_analysis_report_with_corrected_encoding.txt', 'w', encoding='utf-8') as f:
    f.write(new_report_content)

print("\n分析完成！结果已保存到 improved_final_analysis_report_with_corrected_encoding.txt")
print(f"\n信度检验结果: Cronbach's Alpha = {cronbach_alpha:.4f}")
print("相关性分析结果已按正确编码方式更新")